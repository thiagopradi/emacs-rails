...
;; CHANGED some defcustom default values (Lennart Borgman)
...
(defcustom js2-use-font-lock-faces t ;nil
  ...
  (let (tt
    ...
    (when js2-parse-interruptable-p
      (if (zerop (% (incf js2-parse-stmt-count)
          ...
          (progn
            (message "checking pending=%s + changed=%s" (input-pending-p) js2-fl-changed)
